		...
		if($cek > 0){
			...
			$this->session->set_userdata($data_session);
			 redirect("/BNPB");
				...
				$data_session['laporan_fasilitator2']=$this->model_laporan->get_laporan2($data_fasilitator->ID_fasilitator);
				$data_session['jumlah_laporan']=$this->model_laporan->get_proses($data_fasilitator->ID_fasilitator);
				$this->session->set_userdata($data_session);
				redirect("/Fasilitator");
			}else if($data['data_login']->job=='admin'){
				$this->session->set_userdata($data_session);
				redirect('/Admin');
			}else{
			...
			echo $this->session->set_flashdata('msg','<div class="alert alert-danger" style="text-align:center;">Username Atau Password Salah</div>');
			redirect("/Login");
		}
